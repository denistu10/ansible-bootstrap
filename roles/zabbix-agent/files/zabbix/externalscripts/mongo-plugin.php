...
$mem_mapped = $server_status['mem']['mapped'] ;
write_to_data_file("$zabbix_name mem_mapped $mem_mapped") ;
...
$indexCounters_accesses = $server_status['indexCounters']['accesses'] ;
write_to_data_file("$zabbix_name indexCounters_accesses $indexCounters_accesses") ;

$indexCounters_hits = $server_status['indexCounters']['hits'] ;
write_to_data_file("$zabbix_name indexCounters_hits $indexCounters_hits") ;

$indexCounters_misses = $server_status['indexCounters']['misses'] ;
write_to_data_file("$zabbix_name indexCounters_misses $indexCounters_misses") ;

$indexCounters_resets = $server_status['indexCounters']['resets'] ;
write_to_data_file("$zabbix_name indexCounters_resets $indexCounters_resets") ;

$indexCounters_missRatio = $server_status['indexCounters']['missRatio'] ;
write_to_data_file("$zabbix_name indexCounters_missRatio $indexCounters_missRatio") ;
...
$db_info_fileSize = '' ;
$db_info_indexesSize = 0; // Add by Ilya
...
foreach($db_list['databases'] as $db) {
    ...
    $db_info_indexesSize += $db_stats['indexSize']; // Add by Ilya
}
...
write_to_data_file("$zabbix_name db_indexesSize $db_info_indexesSize") ; // Add by Ilya
...
exec("cat $data_file_name 2>&1", $log_file_data) ;
